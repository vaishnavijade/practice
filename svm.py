...
Description:
In this exercise, you will implement a basic Support Vector Machine (SVM) model 
from scratch using only NumPy and Pandas libraries. The goal is to classify a 
synthetic dataset into two categories: Non-diabetic and Diabetic.
...
The dataset contains 200 samples, with 100 samples each for Non-diabetic and 
Diabetic classes. The features are generated synthetically using random sampling, 
with distinct means for each class to ensure separability.
...
3. The "main" method must run and generate output in the format specified in the code, 
   this is important as test cases invoke the main method for evaluation.
'''

import numpy as np
import pandas as pd

# NOTE: This is programmatically generated sample dataset, it does not follow 
#       the standard ranges of the features (Glucose, BMI, Age).
...
def train_svm(X, y, learning_rate=0.01, epochs=1000, lambda_param=0.01):
    ...
    for epoch in range(epochs):
        for idx,x_i in enumerate(X):
            condition=y[idx]*(np.dot(x_i,weights)-bias)>=1   #yi(wi*xi-b)>=1
            if condition:
                weights-=learning_rate*(2*lambda_param*weights)
                #weights−=learning rate×(2×λ×weights)
            else:
